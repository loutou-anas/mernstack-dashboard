...
import NorthSpeciesDataModel from '../models/NorthSpeciesData.js'; // Model for Atlantique North
...
export const uploadNorthData = async (req, res) => {
    try {
        ...
        await NorthSpeciesDataModel.deleteMany({ month, year });
        ...
        // Insert new data
        await NorthSpeciesDataModel.insertMany(rows);
...
export const deleteDataByMonthAndYear = async (req, res) => {
    try {
        ...
        const deleteResult = await NorthSpeciesDataModel.deleteMany({ month, year });
...
// The same logic applies for `uploadSudData` just replace `NorthSpeciesDataModel` with `SudSpeciesDataModel`


export const getAvailableNorthMonthsYears = async (req, res) => {
    try {
        ...
        const monthsYears = await NorthSpeciesDataModel.aggregate([
...
export const getNorthCompositionData = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const compositionData = await NorthSpeciesDataModel.aggregate([
...
export const getNorthEnvironmentalParams = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const environmentalParams = await NorthSpeciesDataModel.aggregate([
            { $match: { month: parseInt(month), year: parseInt(year) } },
            {
                $group: {
                    _id: '$ESPECE',
...
export const getNorthSpeciesResult = async (req, res) => {
    try {
        ...
        const speciesResult = await NorthSpeciesDataModel.aggregate([
...
export const getNorthBiostatData = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const biostatData = await NorthSpeciesDataModel.find({ month, year });
...
export const getNorthSpeciesOptions = async (req, res) => {
    try {
        const speciesOptions = await NorthSpeciesDataModel.distinct('ESPECE');
...
export const getNorthMaturityDataBySpecies = async (req, res) => {
    try {
        const speciesName = req.query.species;

        const maturityData = await NorthSpeciesDataModel.aggregate([
...
export const getNorthSpeciesByMonthYear = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const species = await NorthSpeciesDataModel.distinct('ESPECE', { month: parseInt(month), year: parseInt(year) });
