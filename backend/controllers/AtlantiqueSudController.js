...
export const uploadSudData = async (req, res) => {
    try {
        ...
        await SudSpeciesDataModel.deleteMany({ month, year });
        ...
        // Insert new data
        await SudSpeciesDataModel.insertMany(rows);
...
export const getAvailableSudMonthsYears = async (req, res) => {
    try {
        ...
        const monthsYears = await SudSpeciesDataModel.aggregate([
...
export const getSudCompositionData = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const compositionData = await SudSpeciesDataModel.aggregate([
...
export const getSudEnvironmentalParams = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const environmentalParams = await SudSpeciesDataModel.aggregate([
            { $match: { month: parseInt(month), year: parseInt(year) } },
            {
                $group: {
                    _id: '$ESPECE',
...
export const getSudSpeciesResult = async (req, res) => {
    try {
        ...
        const speciesResult = await SudSpeciesDataModel.aggregate([
...
export const getSudBiostatData = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const biostatData = await SudSpeciesDataModel.find({ month, year });
...
export const getSudSpeciesOptions = async (req, res) => {
    try {
        const speciesOptions = await SudSpeciesDataModel.distinct('ESPECE');
...
export const getSudMaturityDataBySpecies = async (req, res) => {
    try {
        const speciesName = req.query.species;

        const maturityData = await SudSpeciesDataModel.aggregate([
            { $match: { ESPECE: speciesName } },
            {
                $group: {
                    _id: { MAT: "$MAT", SEX: "$SEX" },
                    count: { $sum: "$NB" }  // Sum the number of species (NB) instead of just counting documents
                }
            },
            {
                $group: {
...
export const getSudSpeciesByMonthYear = async (req, res) => {
    try {
        const { month, year } = req.query;
        ...
        const species = await SudSpeciesDataModel.distinct('ESPECE', { month: parseInt(month), year: parseInt(year) });
...
export const deleteDataByMonthAndYear = async (req, res) => {
    try {
        ...
        const deleteResult = await SudSpeciesDataModel.deleteMany({ month, year });
