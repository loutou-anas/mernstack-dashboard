...
const SudBiostatDataPage = () => {
    ...
    const labels = {
        pageTitle: language === 'en' ? 'Biostat - South Atlantic' : 'Biostat - Atlantique Sud',
        ...
        saveButton: language === 'en' ? 'Save Data' : 'Enregistrer les données',
    ...
    const fetchBiostatData = useCallback(async () => {
        ...
        try {
            ...
            const response = await get('/api/atlantique-sud/biostat-data', params);
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm(language === 'en' ? 'Are you sure you want to delete this data?' : 'Êtes-vous sûr de vouloir supprimer ces données?')) {
            try {
                ...
                await delet(`/api/atlantique-sud/delete-data${params}`);
...
export default SudBiostatDataPage;
