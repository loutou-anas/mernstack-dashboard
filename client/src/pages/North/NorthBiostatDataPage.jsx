...
const NorthBiostatDataPage = () => {
    ...
    const labels = {
        pageTitle: language === 'en' ? 'Biostat - North Atlantic' : 'Biostat - Atlantique Nord',
    ...
    const fetchBiostatData = useCallback(async () => {
        ...
        try {
            ...
            const response = await get('/api/atlantique-North/biostat-data', params);
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm(language === 'en' ? 'Are you sure you want to delete this data?' : 'Êtes-vous sûr de vouloir supprimer ces données?')) {
            try {
                ...
                await delet(`/api/atlantique-North/delete-data${params}`);
...
export default NorthBiostatDataPage;
