...
const NorthResultatParEspece = () => {
    ...
    const labels = {
        pageTitle: language === 'en' ? 'Species Result - North Atlantic' : 'Résultat par Espèce - Atlantique Nord',
    ...
    useEffect(() => {
        const fetchSpeciesOptions = async () => {
            if (selectedMonth && selectedYear) {
                try {
                    const response = await get('/api/atlantique-North/species-by-month-year', { month: selectedMonth, year: selectedYear });
    ...
    useEffect(() => {
        const fetchMonthsYears = async () => {
            try {
                const response = await get('/api/atlantique-North/available-months-years');
    ...
    useEffect(() => {
        if (selectedSpecies && selectedMonth && selectedYear) {
            const fetchData = async () => {
                try {
                    const response = await get('/api/atlantique-North/species-result', { species: selectedSpecies, month: selectedMonth, year: selectedYear });
    ...
    useEffect(() => {
        if (selectedSpecies && selectedMonth && selectedYear) {
            const fetchMaturityData = async () => {
                try {
                    const response = await get(`/api/atlantique-North/maturity-data`, { species: selectedSpecies, month: selectedMonth, year: selectedYear });
...
export default NorthResultatParEspece;
