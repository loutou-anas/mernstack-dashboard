...
const ResultatParEspece = () => {
    ...
    const labels = {
        pageTitle: language === 'en' ? 'Result by Species - Mediterranean' : 'Résultat par espèce - Méditerranée',
        ...
        sizeStructureTitle: language === 'en' ? 'Size Structure' : 'Structure des tailles',
        maturityTitle: language === 'en' ? 'Maturity Stages' : 'Stades de Maturité',
        loading: language === 'en' ? 'Loading...' : 'Chargement...',
    };

    // Fetch species based on month and year
    useEffect(() => {
        const fetchSpeciesOptions = async () => {
            if (selectedMonth && selectedYear) {
                try {
                    const response = await get('/api/dashboard/species-by-month-year', { month: selectedMonth, year: selectedYear });
                    setSpeciesOptions(response.data);
                    if (response.data.length > 0) {
                        setSelectedSpecies(response.data[0]);
                    } else {
                        setSelectedSpecies('');  // Clear selection if no species available
                    }
                } catch (error) {
    ...
    useEffect(() => {
        const fetchMonthsYears = async () => {
            try {
                const response = await get('/api/dashboard/available-months-years');
    ...
    useEffect(() => {
        // Filter years when a month is selected
        if (selectedMonth) {
    ...
    useEffect(() => {
        // Filter months when a year is selected
        if (selectedYear) {
    ...
    useEffect(() => {
        if (selectedSpecies && selectedMonth && selectedYear) {
            const fetchData = async () => {
                try {
                    const response = await get('/api/dashboard/species-result', { species: selectedSpecies, month: selectedMonth, year: selectedYear });
    ...
    }, [selectedSpecies, selectedMonth, selectedYear]); // Dependency array to ensure re-fetch on change
    ...
    useEffect(() => {
        if (selectedSpecies && selectedMonth && selectedYear) {
            const fetchMaturityData = async () => {
                try {
                    const response = await get(`/api/dashboard/maturity-data?species=${selectedSpecies}&month=${selectedMonth}&year=${selectedYear}`);
    ...
    const barChartData = useMemo(() => {
        ...
        return {
            ...
            datasets: [{
                label: `${labels.sizeStructureTitle} de la ${selectedSpecies}`,
    ...
    const maturityChartData = useMemo(() => {
        ...
        return {
            labels,
            datasets: [
                { label: 'Stage 1', data: stage1Data, backgroundColor: 'purple' },
                { label: 'Stage 2', data: stage2Data, backgroundColor: 'yellow' },
                { label: 'Stage 3', data: stage3Data, backgroundColor: 'red' },
                { label: 'Stage 4', data: stage4Data, backgroundColor: 'blue' },
                { label: 'Stage 5', data: stage5Data, backgroundColor: 'green' },
            ],
        };
    }, [maturityData]);
            ...
            {data ? (
                <div className="result-content">
                    <div className="chart-container">
                        <h3>{`${labels.sizeStructureTitle} de la ${selectedSpecies}`}</h3>
                        {barChartData && (
                            <Bar
                                data={barChartData}
                                options={{
                                    plugins: {
                                        title: { display: true, text: `${labels.sizeStructureTitle} de la ${selectedSpecies}` },
                                        tooltip: {
                                            callbacks: {
                    ...
                    <div className="chart-container">
                        <h3>{`${labels.maturityTitle} de la ${selectedSpecies}`}</h3>
                        {maturityChartData && (
                            <Bar
                                ...
                                options={{
                                    ...
                                    plugins: {
                                        legend: { position: 'top' },
                                        title: { display: true, text: `${labels.maturityTitle} de la ${selectedSpecies}` },
                                        tooltip: {
                                            callbacks: {
                                                label: function (tooltipItem) {
...
export default ResultatParEspece;
